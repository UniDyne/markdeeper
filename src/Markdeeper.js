...
const { entag, createSlug, escapeHTMLEntities, removeHTMLTags, replaceMatched } = require('./StringUtils');
const { protect, exposeAll }  = require('./StringProtect');

const processFences = require('./FenceProc');
const processTables = require('./TableProc');
const processMedia = require('./MediaProc');
const processSchedules = require('./ScheduleProc');
const processDefinitions = require('./DefinitionProc');
const processLists = require('./ListProc');
...
async function processMarkdown(str, options) {
    if(options) module.exports.init(options);
...
module.exports = {
    init: function(options) { Object.assign(MARKDEEP_CONFIG, options); },
    ...
    processBook: async function(str) {
        return await processMarkdown(str, {asDoc:true});
    }
};
